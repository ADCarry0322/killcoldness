...
@interface ViewController ()<KeyboardDelegate,UITableViewDelegate,UITableViewDataSource,UIGestureRecognizerDelegate,NSCoding>
...
//    NSString *time=[[NSString alloc] initWithFormat:@"%2ld:%2ld",(long)hour,(long)minute];
    ...
    self.arrList=msgStore.privateMessages;
}


//发送的文本
-(void)textViewContentText:(NSString *)textStr{
    MessageModel *userMessage=[[MessageModel alloc]init];
    userMessage.messageText=textStr;
    userMessage.messageSenderType=MessageSenderTypeUser;
    userMessage.showMessageTime=NO;
    userMessage.messageType=MessageTypeText;
    
    MessageStore *msgStore=[MessageStore sharedStore];
    [msgStore.privateMessages addObject:userMessage];
    //[self.arrList addObject:userMessage];
    //msgStore.privateMessages=_arrList;
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    if ([userDefaults stringForKey:@"name"]) {
        
        self.navigationItem.title=[userDefaults stringForKey:@"name"];
...
-(void)clickToSettingPage{
    ...
    [defaults setObject:self.navigationItem.title forKey:@"title"];
